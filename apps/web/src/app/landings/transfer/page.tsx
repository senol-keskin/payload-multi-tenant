...
export default async function TransferLandingPage() {
  ...
  const populerTransferPoints = widgets?.filter(
    (x) => x.point == 'populer_transfer_React'
  )
              ...
              {populerTransferPoints.map((destination) => {
                return (
                  <ProductBox
                    ...
                    url={
                      (destination.params.link.value &&
                      destination.params.link.value.length > 0
                        ? `/transfer/search-results?${destination.params.link.value}`
                        : '') as Route
                    }
                  />
                )

  ...
  return results
    ...
    .filter((tourData) => {
      if (filterParams.tourCities && filterParams.tourCities.length > 0) {
        const mainItemHasTourCity = tourData?.cities?.some((tourCity) =>
          filterParams.tourCities?.includes(tourCity.code ?? '')
        )
        const relatedItemsHaveTourCity = tourData?.relatedItems?.some(
          (relatedItem) =>
            relatedItem?.cities?.some((tourCity) =>
              filterParams.tourCities?.includes(tourCity.code ?? '')
            )
        )
        return mainItemHasTourCity || relatedItemsHaveTourCity
      }
      return true
    })

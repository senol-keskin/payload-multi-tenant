...
export const TourSearchResultItem: React.FC<Props> = ({
  ...
  return (
    <div className='grid grid-cols-1 rounded-md border border-gray-300 shadow md:grid-cols-5'>
      <div className='grid gap-3 md:col-span-4 md:gap-5'>
        <div className='grid grid-cols-12 items-stretch gap-4 p-3'>
                  ...
                  .map((campaign) => (
                    <Carousel.Slide key={campaign.id}>
                      <Link href={campaign.params.link?.value as Route}>
                        <div className='rounded-md border-l-4 border-l-blue-800 bg-blue-100 px-3 py-2 hover:bg-blue-200'>
                          {campaign.title}
                        </div>
                      </Link>
                    </Carousel.Slide>
      ...
      {campaignContents?.filter(
        (campaign) =>
          campaign.params.view_country.value === String(selectedTour.isDomestic)
      ).length > 0 ? (
